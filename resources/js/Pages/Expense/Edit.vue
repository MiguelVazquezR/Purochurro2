...
const form = useForm({
    ...
    date: new Date(props.expense.date.split('T')[0] + 'T00:00:00'),
...
const submit = () => {
    form.transform((data) => ({
        ...data,
        // Al guardar, formateamos manualmente a YYYY-MM-DD usando la fecha local
        // data.date.toISOString() usaría UTC y podría cambiarte el día de nuevo.
        date: data.date ? 
        ...
        onSuccess: () => {
            toast.add({ severity: 'success', summary: 'Actualizado', detail: 'Gasto actualizado correctamente', life: 3000 });
        },
        onError: () => {
            ...
            <div class="flex-1 flex flex-col gap-8 w-full min-w-0">
                ...
                <div class="sticky top-16 z-30 bg-surface-50/90 backdrop-blur-md py-4 border-b border-surface-200/50 flex items-center justify-between transition-all duration-300">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold tracking-tight text-surface-900">Editar gasto</h1>
                            <p class="text-surface-500 text-sm mt-1 hidden md:block">Modifica la información del movimiento.</p>
                        </div>
                    </div>
