            ...
            <div class="flex-1 flex flex-col gap-8 w-full min-w-0">
                <div
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold tracking-tight text-surface-900">Nuevo producto</h1>
                            <p class="text-surface-500 text-sm mt-1 hidden md:block">Registra un nuevo ítem en el
                                catálogo.
                            </p>
                        </div>
                    </div>
                    <div class="flex gap-2">
                        ...
                        <Button label="Guardar" icon="pi pi-check" @click="submit" :loading="form.processing"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-10">
                    ...
                    <div class="lg:col-span-1 flex flex-col gap-6">
                        ...
                        <div
                            ...
                            <span class="text-sm font-semibold text-surface-700 mb-4 self-start">Imagen del
                                producto</span>
                            <div class="relative group cursor-pointer w-48 h-48">
                                ...
                                <Button v-if="imagePreview" icon="pi pi-times" rounded severity="danger"
                                    class="absolute -top-4 -right-2 !size-6 shadow-md" @click.stop="removeImage" />
                            </div>
                            <small class="text-surface-400 text-xs mt-3">PNG, JPG hasta 2MB.</small>
                            <small v-if="form.errors.image" class="text-red-500 text-xs mt-1">{{ form.errors.image
                                }}</small>
                        </div>
                        ...
                        <div class="bg-white rounded-3xl shadow-sm border border-surface-200 p-6 flex flex-col gap-5">
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex flex-col"><span class="text-sm font-medium text-surface-900">Estado
                                        activo</span><span class="text-xs text-surface-500">Visible en el sistema</span>
                                </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex flex-col"><span
                                        class="text-sm font-medium text-surface-900">Vendible</span><span
                                        class="text-xs text-surface-500">Aparece en punto de venta</span></div>
                                <ToggleSwitch v-model="form.is_sellable" />
                            </div>
                            <hr class="border-surface-100" />
                            <div class="flex items-center justify-between">
                                <div class="flex flex-col"><span class="text-sm font-medium text-surface-900">Controlar
                                        stock</span><span class="text-xs text-surface-500">Restar inventario al
                                        vender</span></div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="flex flex-col gap-2 md:col-span-2">
                                    ...
                                    <InputText id="name" v-model="form.name" class="w-full"
                                        placeholder="Ej. Hamburguesa Doble Queso" :invalid="!!form.errors.name" />
                                ...
                                <div class="flex flex-col gap-2 md:col-span-2">
                                    ...
                                    <div class="flex gap-2">
                                        <Select id="category" v-model="form.category_id" :options="categories"
                                            optionLabel="name" optionValue="id" placeholder="Seleccionar categoría"
                                            class="w-full" :invalid="!!form.errors.category_id" filter>
                                            <template #option="slotProps">
                                                <div class="flex items-center gap-2">
                                                    <div class="w-3 h-3 rounded-full"
                                                        :style="{ backgroundColor: slotProps.option.color || '#ccc' }">
                                                    </div>
                                ...
                                <div class="flex flex-col gap-2">
                                    <label for="barcode" class="font-medium text-surface-700">Código de barras
                                        (Opcional)</label>
                                    <IconField>
                                        <InputIcon class="pi pi-barcode" />
                                        <InputText id="barcode" v-model="form.barcode" class="w-full"
                                            placeholder="Escanea o escribe..." :invalid="!!form.errors.barcode" />
                                    </IconField>
                                    <small v-if="form.errors.barcode" class="text-red-500">{{ form.errors.barcode
                                        }}</small>
                                </div>
                                <div class="flex flex-col gap-2 md:col-span-2">
                                    <label for="description" class="font-medium text-surface-700">Descripción</label>
                                    <Textarea id="description" v-model="form.description" rows="3"
                                        class="w-full !resize-none" placeholder="Detalles del producto..." />
                        ...
                        <div class="bg-white rounded-3xl shadow-sm border border-surface-200 p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div class="flex flex-col gap-2">
                                    ...
                                    <InputNumber id="price" v-model="form.price" mode="currency" currency="MXN"
                                        locale="es-MX" class="w-full" inputClass="!font-bold !text-surface-900"
                                        placeholder="$0.00" :invalid="!!form.errors.price" fluid />
                                    <small v-if="form.errors.price" class="text-red-500">{{ form.errors.price }}</small>
                                </div>

                                <div class="flex flex-col gap-2">
                                    <label for="employee_price" class="font-medium text-surface-700">Precio
                                        empleado</label>
                                    <InputNumber id="employee_price" v-model="form.employee_price" mode="currency"
                                        currency="MXN" locale="es-MX" class="w-full" placeholder="$0.00"
                                        :invalid="!!form.errors.employee_price" />
                                    <small v-if="form.errors.employee_price" class="text-red-500">{{
                                        form.errors.employee_price }}</small>
                                </div>

                                <div class="flex flex-col gap-2">
                                    <label for="cost" class="font-medium text-surface-700">Costo (insumos)</label>
                                    <InputNumber id="cost" v-model="form.cost" mode="currency" currency="MXN"
                                        locale="es-MX" class="w-full" placeholder="$0.00"
