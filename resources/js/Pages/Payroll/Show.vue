...
<template>
    <AppLayout title="Detalle de Nómina">
        ...
        <div class="w-full flex flex-col gap-6 h-full p-2 md:p-0">
            ...
            <div
                ...
                <div class="flex items-center gap-3 rounded-full px-2 py-1 border border-surface-100 bg-surface-50">
                    ...
                    <div class="text-center px-2">
                        <span class="block text-[10px] text-surface-500 uppercase tracking-wider font-bold">Periodo
                            Semanal</span>
                        <span class="text-base md:text-lg font-bold text-surface-900 capitalize whitespace-nowrap">{{
                            dateRangeLabel }}</span>
                    </div>
                ...
                <div class="w-full max-w-md">
                    <IconField iconPosition="left">
                        ...
                        <InputText v-model="search" placeholder="Buscar empleado..."
                            class="w-full !rounded-full border-none bg-surface-100 focus:bg-white transition-colors focus:ring-0" />
                    </IconField>
                </div>
            ...
            <!-- ============================================== -->
            <div
                class="hidden md:flex bg-white border border-surface-200 rounded-3xl shadow-xl overflow-hidden flex-1 flex-col relative">
                <div class="overflow-x-auto custom-scrollbar flex-1">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr
                                class="bg-surface-50 text-surface-600 text-xs uppercase tracking-wider border-b border-surface-200">
                                <th
                                    class="p-4 font-bold sticky left-0 bg-surface-50 z-20 min-w-[250px] shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                                    Empleado</th>

                                <template v-if="payrollData.length > 0">
                                    <th v-for="day in payrollData[0].days" :key="day.date"
                                        class="p-3 text-center min-w-[140px]">
                                        <div class="flex flex-col items-center">
                                            <span class="font-bold">{{ day.day_name.substring(0, 3) }}</span>
                                            <span class="text-[10px] text-surface-400 px-2 rounded-full">{{
                                                dayjs(day.date).format('D MMM') }}</span>
                                        </div>
                                    </th>
                                </template>

                                <th
                                    class="p-4 text-center font-bold min-w-[115px] bg-surface-50 sticky right-0 z-10 shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                                    Total</th>
                            </tr>
                        </thead>
                        <tbody class="text-sm divide-y divide-surface-100">
                            <tr v-for="row in filteredEmployees" :key="row.employee.id"
                                class="group hover:bg-surface-50/50 transition-colors">

                                <!-- Columna Empleado -->
                                <td
                                    class="p-4 sticky left-0 bg-white group-hover:bg-surface-50 transition-colors z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                                    <div class="flex items-center gap-3">
                                        <div class="relative flex-shrink-0">
                                            <img v-if="row.employee.profile_photo_url"
                                    ...
                                    <div class="h-24 rounded-xl border p-2 flex flex-col justify-center items-center text-center cursor-pointer transition-all hover:scale-[1.02] hover:shadow-md relative overflow-hidden"
                                        ...
                                        <div class="absolute top-1 right-1.5 flex gap-1" v-else>
                                            <div v-if="day.is_late && !day.late_ignored"
                                                ...
                                                <div class="size-2 rounded-full bg-red-500 ring-2 ring-white"
                                                    title="Retardo"></div>
                                            </div>
                                            <div v-if="day.late_ignored"
                                        ...
                                        <!-- Turno -->
                                        <div
                                            class="text-[9px] uppercase font-bold opacity-60 mb-1 mt-2 w-full truncate">
                                            {{ day.schedule_shift || '' }}
                                        </div>
                                        ...
                                        <template v-else-if="day.incident_type === 'asistencia' && day.check_in">
                                            <div class="flex flex-col gap-0">
                                                <span
                                                    class="text-[11px] font-bold text-surface-800 bg-white/50 px-1 rounded whitespace-nowrap">{{
                                                        formatTime12h(day.check_in) }}</span>
                                                <span class="text-[10px] text-surface-600 whitespace-nowrap">{{
                                                    formatTime12h(day.check_out) || '--:--' }}</span>
                                            </div>
                                        </template>
                                ...
                                <!-- Total Horas -->
                                <td
                                    class="p-4 text-center font-bold text-surface-700 bg-white sticky right-0 z-10 shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.05)] border-l border-surface-100">
        ...
        <Dialog v-model:visible="editDialog" modal :header="selectedEmployee ? `Editar registro` : 'Gestionar'"
            :style="{ width: '32rem' }" :breakpoints="{ '960px': '75vw', '641px': '90vw' }" class="p-fluid"
            :draggable="false">
            <div class="flex flex-col gap-5 pt-2" v-if="selectedDay && selectedEmployee">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <Select v-model="form.incident_type" :options="incidentOptions" optionLabel="label"
                        optionValue="value" placeholder="Selecciona estado" class="w-full"
                ...
                <div v-if="form.incident_type === 'asistencia'"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="flex flex-col gap-2">
                            ...
                            <DatePicker v-model="form.check_in" timeOnly hourFormat="12" showIcon iconDisplay="input"
                                placeholder="--:-- --" fluid />
                        </div>
                        <div class="flex flex-col gap-2">
                            <label class="font-bold text-surface-700 text-sm">Salida</label>
                            <DatePicker v-model="form.check_out" timeOnly hourFormat="12" showIcon iconDisplay="input"
                                placeholder="--:-- --" fluid />
                        </div>
                    </div>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <Textarea v-model="form.admin_notes" rows="3"
