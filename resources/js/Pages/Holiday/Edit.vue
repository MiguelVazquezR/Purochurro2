...
const form = useForm({
    ...
    // Corrección de fecha para evitar desfase de zona horaria
    date: new Date(props.holiday.date),
    mandatory_rest: Boolean(props.holiday.mandatory_rest),
    pay_multiplier: parseFloat(props.holiday.pay_multiplier),
});
...
const submit = () => {
    form.transform((data) => ({
        ...
        date: data.date ? 
            ...
            : null,
    })).put(route('holidays.update', props.holiday.id), {
        onSuccess: () => {
            toast.add({ severity: 'success', summary: 'Actualizado', detail: 'Día feriado actualizado correctamente', life: 3000 });
        },
        onError: () => {
...
<template>
    <AppLayout title="Editar Feriado">
            ...
            <div class="flex-1 flex flex-col gap-8 w-full min-w-0">
                ...
                <div class="sticky top-16 z-30 bg-surface-50/90 backdrop-blur-md py-4 border-b border-surface-200/50 flex items-center justify-between transition-all duration-300">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold tracking-tight text-surface-900">Editar feriado</h1>
                            <p class="text-surface-500 text-sm mt-1 hidden md:block">Modifica los detalles de la festividad.</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <InputNumber 
                                ...
                                class="w-full"
                            />
                            <small class="text-surface-400 text-xs">Ej. 2.0 (Doble) para días oficiales.</small>
